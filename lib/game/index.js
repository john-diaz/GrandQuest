const gameState = require('./gameState');
const pool = require('../db/client');

module.exports = namespace => {
  var gameLoop = null;

  var stateUpdateInterval = { date: 0, update: 400 };
  var clientUpdateInterval = { date: 0, update: 1000 };

  const gameTick = () => {
    const now = Date.now();

    // update the client
    if (now - clientUpdateInterval.date > clientUpdateInterval.update) {
      namespace.emit('WORLD_STATE', gameState.getState());

      clientUpdateInterval.date = Date.now();
    }
    // update the state
    if (now - stateUpdateInterval.date > stateUpdateInterval.update) {
      // update time of day
      gameState.dispatch({ type: 'ADD_TICK' });

      stateUpdateInterval.date = Date.now()
    }
  }
  const startGameLoop = () => {
    if (gameLoop) {
      throw new Error('Attempted to initiate game loop while one already ran');
    }

    console.log('Started game clock');

    gameLoop = true;
    let connections = gameState.getState().connections;
    const gameClock = setInterval(() => {
      if (Object.keys(connections).length) {
        gameTick();
      } else {
        clearInterval(gameClock);
        gameLoop = null;
        console.log('Ended game clock');
      }
    }, 10);
  }

  console.log('initial connections = ', gameState.getState().connections);

  namespace.on('connect', (socket) => {
    const state = gameState.getState();
    const beforeDispatch = Object.keys(state.connections).length;
    gameState.dispatch({ type: 'CONNECT_USER', payload: socket.id });
    const afterDispatch = Object.keys(state.connections).length;
    
    console.log('connected ', socket.id);
    if (beforeDispatch == 0 && afterDispatch > 0) {
      startGameLoop();
    }
    socket.on('AUTHENTICATE_SOCKET', (token) => {
      if (typeof token !== 'string' && token.trim().length < 10) {
        return
      }
      let userInState = state.connections[socket.id];
      if (!token || !userInState || userInState.authenticated) {
        return
      }

      pool.query('SELECT * FROM users WHERE token = $1', [token], (err, results) => {
        if (err || !results.rowCount) {
          return
        }

        // TODO: check if the user has already authenticated by the user.id in our state
        
        gameState.dispatch({
          type: 'SET_USER_AUTHENTICATED', 
          payload: {
            socketID: socket.id,
            user: results.rows[0],
          }
        });
      });
    });
    socket.on('disconnect', () => {
      console.log('disconnected ', socket.id);
      gameState.dispatch({ type: 'DISCONNECT_USER', payload: socket.id });
    });
  });
}
