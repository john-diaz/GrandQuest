...
const pathToUsers = path.join(pathToStorage, 'users');
...
if (fs.existsSync(pathToStorage)) {
  ...
  */
  if(fs.existsSync(pathToUsers)) {
    let files = fs.readdirSync(pathToUsers)
    for (const file of files) {
      fs.unlinkSync(path.join(pathToUsers, file));
    }
  } else {
    console.warn('$ STORAGE - Missing users directory');
  }
  /*
  ...
  namespace.on('connect', (socket) => {
    ...
    */
    ...
    socket.on('AUTHENTICATE_SOCKET', (token, cb) => {
      ...
      pool.query('SELECT * FROM users WHERE token = $1', [token], (err, results) => {
        if (err || !results.rowCount) {
          if (typeof cb === 'function') cb('Could not find any users for these credentials');
          return;
        }
        
        let user = results.rows[0];
        
        let pathToUser = path.join(pathToUsers, `${user.id}.json`);
        let existingUser = fs.existsSync(pathToUser);
        ...
        fs.writeFileSync(pathToUser, JSON.stringify(user));
    ...
    socket.on('disconnect', () => {
      if (socket.userID) {
        let pathToUser = path.join(pathToUsers, `${socket.userID}.json`);

        fs.unlink(pathToUser, (err) => {
          if (err) throw err;
