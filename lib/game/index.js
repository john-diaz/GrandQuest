    ...
    */
    ...
    socket.on('AUTHENTICATE_SOCKET', (token, cb) => {
      ...
      pool.query('SELECT * FROM users u INNER JOIN players p ON u.id = p.id WHERE token = $1', [token], (err, results) => {
        ...
        let player = _.omit(results.rows[0], (v, key) => _.contains([ 'hashed_password', 'email' ], key));
        ...
        async.waterfall([
          ...
          callback => fs.writeFile(pathToPlayer, '{}', callback),
          // add file watcher for updates
          callback => {
          ...
          // make first update
          callback => fs.writeFile(pathToPlayer, JSON.stringify(player), callback),
        ], (err) => {
          if (err) throw new Error('Error with FS during socket authentication', err);
    ...
    socket.on('disconnect', () => {
      if (socket.userID) {
        ...
        fs.readFile(pathToPlayer, { encoding: 'utf8' }, (err, data) => {
          ...
          pool.query(`UPDATE players SET gold = $1 WHERE id = ${socket.userID}`, [player.gold], (err) => {
            if (err) throw err;
