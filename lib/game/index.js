...
module.exports = io => {
  ...
  let combat = require('./world/combat')(worldStore);
  ...
  const gameTick = () => {
    ...
    if (now - clientUpdateInterval.date > clientUpdateInterval.update) {
      namespace.emit('WORLD_STATE', worldStore.getState().world);
  ...
  const startGameLoop = () => {
    ...
    const gameClock = setInterval(() => {
      let connections = worldStore.getState().world.connections;
  ...
  namespace.on('connect', (socket) => {
    ...
    socket.on('CONNECT_TO_COMBAT', () => {
      let authenticatedUser = fetchSocketUser(socket);
      if (authenticatedUser) {
        combat(socket);
      } else {
        console.log('Socket attempted to join combat but is not authenticated');
      }
    });
