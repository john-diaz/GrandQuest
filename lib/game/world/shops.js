		...
		actions: {
			'heal': (player) => new Promise((resolve, error) => {
				...
				.then((results) => {
					...
					if (health === maxHealth) {
						return resolve('Great news, your health is already maxed out!');
					}
					...
					if (affordableHP < 1) {
						resolve('Sorry, your gold is too low to afford healing.');
					} else {
						let added = Math.min(health + affordableHP, maxHealth) - health;
						let cost = Number((added / goldToHealthUnit).toFixed(2));
