  ...
  _.forEach(roomState.turnEvents, (events) => {
    _.forEach(events, (event) => {
      ...
      if (action.type === 'attack') {
        ...
        if (!character.enemy) {
          record.players[characterId].damageDealt += damage;
      ...
      case 'attack':
        ...
        if (receiver.entity.health <= 0) {
          ...
          if (!character.enemy) {
            ...
            let leveled = 0;
            let newXp = player.level + receiver.entity.xp;
            ...
            pool.query(`
              ...
              WHERE id = ${player.id}
            `, (err) => {
              console.log('saved xp : ', err ? err : 'No error!');
            });
          }
        }
        ...
        console.log('pushed ', {...event, outcome})
        break;
      case 'item':
