...
const saveLevelChanges = (roomState) => {
  ...
  */
    ...
    _.forEach(events, (event) => {
      ...
      if (action.type === 'attack') {
        ...
        if (action.id === 'heal-potion') {
          ...
          const goldReward = outcome.heal * 5 * teammateMultiplier;
          ...
          record[characterId].healed[receiverId].times++;
          record[characterId].healed[receiverId].reward += goldReward;
          record[characterId].healed[receiverId].total += outcome.heal;
          record[characterId].gold += goldReward;
        }
      }
    });
  ...
  _.forEach(record, (userRecord, id) => {
    ...
    // update user gold on new state
    let player = newState.players[id];
    console.log('prev gold ', player.gold);
    player.gold += userRecord.gold;
    console.log('new gold = ', player.gold);
    // update user gold on file system
    fs.writeFileSync(path.join(__dirname, '..', '..', 'storage', 'players', `${id}.json`), JSON.stringify(player));
  });
