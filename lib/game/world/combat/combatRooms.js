...
module.exports = () => (socket) => {
  ...
  */
  ...
  socket.on('COMBAT_ROOM_CONNECT', (roomID, cb) => {
    ...
    */
    if (!socket.userID) {
      ...
      return console.log('no socket player id');
    }
      ...
      */
      ...
      else // Add the player
      {
        ...
        newRoom.players[socket.userID] = {
          ...
          enemy: false,
          entity: Adventurer(), // set this to the role the player is doing
          gold: player.gold,
          selectionStatus: 0,
        };
  ...
  socket.on('COMBAT_ROOM_LEAVE', (cb) => {
    ...
    // update room state
    fs.readFile(pathToCombatRoom, { encoding: 'utf8' }, (err, data) => {
      if (err) throw err;
      let room = JSON.parse(data);
      ...
      // save changes
      fs.writeFile(pathToCombatRoom, JSON.stringify(room), (err) => {
        if (err) throw err;
