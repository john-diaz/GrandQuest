...
module.exports = () => (socket) => {
  ...
  */
   ...
   * @param {string} event.action.id - The ID of the selected action
   */
  socket.on('COMBAT_ROOM_ACTION', (event) => {
    ...
    */
    switch(action.type) {
      case 'attack':
        if (!receiver.enemy) {
          return 'Player cannot attack other players';
        }
        if (!player.entity.attacks[action.id]) {
          return 'Player does not have attack ' + action.id;
        }
        break;
      case 'potion':
